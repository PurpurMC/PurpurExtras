...
import org.purpurmc.purpurextras.modules.impl.listeners.ShieldCooldownListener;
import org.purpurmc.purpurextras.modules.impl.listeners.ShieldDamageReductionListener;
 ...
 */
public class ShieldSettingsModule implements PurpurExtrasModule {
    ...
    public boolean shouldEnable() {
        shieldDamageReduction = getConfigDouble("settings.shield.damage-reduction", shieldDamageReduction);
        shieldDamageReduction = Math.min(1.0, Math.max(0, shieldDamageReduction));

        shieldCooldown = getConfigInt("settings.shield.cooldown", shieldCooldown);
